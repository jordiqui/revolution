...
// Load the fixed HypnoS Zobrist table after the Zobrist namespace has been
// defined so the generated data can populate the arrays above.
#include "revolution_zobrist.h"
...
void Position::init() {
    ...
    // Use the fixed HypnoS Zobrist table so experience files generated by
    // HypnoS remain compatible with Revolution runs. This keeps the keys
    // stable across builds and allows seamless sharing of experience data.
...
inline void add_dirty_threat(
  DirtyThreats* const dts, Piece pc, Piece threatened, Square s, Square threatenedSq) {
    // Only adjust DirtyThreats bitboards when adding a piece. This mirrors
    // Stockfish commit c109a88e, which ensures removals don't touch these
    // bitboards.
