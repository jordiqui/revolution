...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // At non-PV nodes we check for an early TT cutoff. Note that we always check
    // the validity of the TT value because of access races.
    ...
    {
        // If a window-bound mismatch is the only reason cutoff failed, penalize
        // the now-useless TT entry
        ttWriter.penalize(1);
    }
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (rootNode)
        {
            ...
            if (moveCount == 1 || value > alpha)
            {
                ...
                rm.inexactLower = rm.inexactUpper = false;
...
void SearchManager::output_pv(Search::Worker&           worker,
    ...
    for (usize i = 0; i < multiPV; ++i)
    {
        ...
        if (is_decisive(v) && std::abs(v) < VALUE_MATE_IN_MAX_PLY && !usePreviousScore
            && ((!rootMoves[i].inexactLower && !rootMoves[i].inexactUpper) || isExact))
