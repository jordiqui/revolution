...
std::string format_bytes(std::uintmax_t bytes, int decimals = 2) {
...
bool PolyglotBook::open(const std::string& f) {
    ...
    std::error_code ec;
    const auto       currentSize = fs::file_size(f, ec);

    if (!filename.empty() && f == filename && !ec
        && bookDataLength == static_cast<std::size_t>(currentSize))
        return true;
    ...
    if (!inData)
    {
        sync_cout << "info string Could not allocate " << format_bytes(fm.data_size(), 2)
