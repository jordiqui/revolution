...
namespace Stockfish
{
    ...
    namespace Book::Polyglot
    {
        ...
        std::string PolyglotBook::type() const
        {
            return "BIN";
        }
        ...
        bool PolyglotBook::open(const std::string& f)
        {
            ...
            std::error_code ec;
            const auto      fileSize = fs::file_size(f, ec);

            if (!ec && f == filename && bookDataLength == fileSize)
                return true;
            ...
            auto* inData = static_cast<unsigned char*>(std::malloc(fm.data_size()));
            if (!inData)
            {
                sync_cout << "info string Could not allocate " << fm.data_size() << " bytes for reading book file: " << f
                          << sync_endl;
                return false;
            }

            std::memcpy(inData, fm.data(), fm.data_size());
        ...
        void PolyglotBook::show_moves(const Position& pos) const
        {
            ...
            else
            {
                ...
                {
                    std::stable_sort(bookMoves.begin(), bookMoves.end(), [](const PolyglotBookMove& bm1, const PolyglotBookMove& bm2) { return bm1.entry.count > bm2.entry.count; });
