...
bool PolyglotBook::open(const std::string& f) {
    ...
    if (::Stockfish::Book::is_same_file(f, filename)
        && ::Stockfish::Book::get_file_size(f) == bookDataLength)
        return true;
    ...
    FileMapping fm;
    if (!fm.map(::Stockfish::Book::map_path(f), false))
    {
    ...
    if (!inData)
    {
        sync_cout << "info string Could not allocate "
                  << ::Stockfish::Book::format_bytes(fm.data_size(), 2)
