...
namespace Stockfish
{
    ...
    namespace Book::CTG
    {
        ...
        bool CtgBook::open(const std::string& f)
        {
            ...
            fs::path base = fs::u8path(f);
            base.replace_extension(".ctg");
            const auto ctgFile = base.u8string();

            if (!ctg.map(ctgFile, true))
            {
                close();
            ...
            auto ctoFile = base;
            ctoFile.replace_extension(".cto");
            if (!cto.map(ctoFile.u8string(), true))
            {
                close();

                sync_cout << "info string Could not open CTO file: " << ctoFile.u8string() << sync_endl;
                return false;
            }
            ...
            if (!ctb.map(ctbFile.u8string(), true))
            {
                ...
                sync_cout << "info string Could not open CTB file: " << ctbFile.u8string() << sync_endl;
                return false;
            }
